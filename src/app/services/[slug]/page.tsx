...
export default function ServicePage({ params }: { params: { slug: string } | Promise<{ slug: string }> }) {
  ...
  return (
    <div className='mx-auto max-w-6xl px-4 sm:px-6'>
        <ServicesHero title={service.title} description={service.description}/>
        <div className="mt-8 sm:mt-10 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 sm:gap-6">
            {service?.features.map((feature, index) => (
                <article
                    key={index}
                    onClick={() => openServicesModal()}
                    className="cursor-pointer overflow-hidden rounded-xl border border-slate-200 bg-white shadow-[0_5px_12px_rgba(0,0,0,0.06)] hover:shadow-[0_8px_18px_rgba(0,0,0,0.12)] transition-all duration-300 hover:-translate-y-1 hover:border-blue-300"
                >
                    ...
                    <div className="mt-4 text-blue-700 font-bold text-xl">{service.price}</div>
                    </div>
                </article>
            ))}
            </div>

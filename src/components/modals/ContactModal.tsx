...
export default function ContactModal({ open, onClose }: ContactModalProps) {
  return (
    <Modal 
      ...
      className="max-w-2xl" 
    >
      <h3 className="text-center text-2xl md:text-3xl font-bold text-slate-900 mb-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="flex flex-col text-center">
          <div className="flex justify-center align-center gap-2 text-slate-900 font-semibold w-full">
            <Location /> <span className="pt-0.5 pl-1">Волгоград, ул. Елецкая 8/1</span>
          </div>
        ...
        <div className="flex flex-col text-center">
          <div className="flex justify-center items-center gap-2 text-slate-900 font-semibold w-full">
             <Location /> <span className="pt-0.5 pl-1">Волгоград, ул. Качинцев д.95</span>
          </div>
      ...
      <div className="mt-6 flex items-center justify-center gap-3">
        ...
        {[{label: "Facebook", icon: <Facebook />}, {label: "VK", icon: <VK />}] .map((s) => (
          <button
            key={s.label}
            aria-label={s.label}
            className="h-10 w-10 rounded-md bg-white text-slate-800 grid place-items-center border border-slate-200 hover:bg-slate-50"
